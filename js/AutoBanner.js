~function () {
    ...
    AutoBanner.prototype = {
        ...
        bindData: function () {
            var str = "", str2 = "";
            //this.jsonData = this.jsonData['bannerImg'];
            if (this.jsonData) {
                for (var i = 0, len = this.jsonData.length; i < len; i++) {
                    var curData = this.jsonData[i];
                    str += "<a><img src='' trueImg='" + curData["img"] + "'/></a>";
                    i === 0 ? str2 += "<li class='bg'><a href='#'>"+curData["text"]+"</a></li>" : str2 += "<li><a href='#'>"+curData["text"]+"</a></li>";
                }
            }
        ...
        setBanner: function () {
            //->实现轮播图切换效果
            for (var i = 0, len = this.divAList.length; i < len; i++) {
                var curDiv = this.divAList[i];
                if (i === this.step) {
